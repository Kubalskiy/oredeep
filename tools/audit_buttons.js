...
const shells = fs.readFileSync(path.join(__dirname, "audit_ui_shells.js"), "utf8");
const ids = [...html.replace(game, "").matchAll(/id="([A-Za-z_]+)"/g)].map(m => m[1]);
globalThis.__HTML_IDS = ids;
globalThis.__RAW_HTML = html;
eval(stub + game + ui + cases + shells);
process.exit((globalThis.__auditFail || globalThis.__uiShellFail) ? 1 : 0);
